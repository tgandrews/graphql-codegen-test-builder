...
type TypeDefMap<T extends GQLKind = GQLKind> = {
  [K in T]: {
    renderDefaultValue(field: FieldValue & { type: Extract<GQLType, { kind: K }> }, parseResult: ParseResult): string
    renderType(field: FieldValue & { type: Extract<GQLType, { kind: K }> }, parseResult: ParseResult): string
  }
}
...
const renderOutputField = (field: FieldValue, parseResult: ParseResult, parentPath: string[] = []): string => {
